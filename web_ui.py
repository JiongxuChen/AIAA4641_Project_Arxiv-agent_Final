...
WEB_AGENT = ResearchBriefingAgent(papers_library=PAPERS_LIB, task_history=TASK_HIST)
...
def run_retrieval_api():
    ...
    check_existing = data.get('checkExisting', False)
    add_to_library = data.get('addToLibrary', False)
...
def run_followup_api():
    ...
    return jsonify(WEB_AGENT.answer_followup_once(
        ...
        llm_model=data.get('llm_model', 'deepseek-ai/DeepSeek-R1'),
...
def run_full_task():
    data = request.json
    query = data.get('query')
    task_type = data.get('task_type', 'full_pipeline')
    run_mode = data.get('run_mode', 'now')
    schedule_time = data.get('schedule_time', '')
    days = data.get('days', 7)
    max_results = data.get('max_results', 20)
    add_to_library = data.get('add_to_library', True)
